...
public class Spline {
	...
	public static final int TWOPOINTS=15000;
	int n, last_interval;
	double x[], f[], b[], c[], d[];
	...
	public Spline(double xx[], double ff[]) {
		...
		if (n <= 3) {
			...
			Log.d(TAG, "not enough points to build spline, n=" + n);
			return;
		}
		if (n != ff.length) {
			//System.out.println("not same number of x and f(x)");
			Log.d(TAG,"not same number of x and f(x)");
			return;
		}
		x = new double[n];
		f = new double[n];
		...
		for (int i = 0; i < n; i++) {
			...
			if (debug)
				...
				Log.d(TAG, "Spline data x[" + i + "]=" + x[i]+ ", f[]=" + f[i]);
			if (i >= 1 && x[i] < x[i - 1])
				sorted = false;
		}
		// sort if necessary
